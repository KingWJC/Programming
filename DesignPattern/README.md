...
实现：普通代理(给代理对象指定真实对象,不允许直接创建真实角色)，强制代理(真实对象指定代理对象,不允许直接访问真实角色)，动态代理(反射动态生成).

Java实现动态代理: 

- asm在运行时,动态生成二进制.  (时序图)
- Instrument 拦截class加载方法,处理二进制. 
- CGLIB. 被代理类无法用final修饰.  底层asm.
...
优点：把类的核心职责和装饰功能分离，去除相关类中重复的装饰逻辑。

NET：BufferedStream，CryptoStream等式具体装饰类，为FileStream，MemoryStream动态增加功能，省略了抽象装饰角色。

#### 复合模式

定义：将对象组合成树形结构以表示“部分-整体‘的层次结构，使用户对单个都西昂和组合对象的使用具有一致性。
...
目的：针对软件的运行质量，对于值语义对象，通过共享机制，减少对象的创建，
...
2. 静态分派：分派能在编译期决定。如重载多态和参数多态。

3. 动态分派：分派能在运行期决定。如子类型多态（在运行期间，根据一个方法的接收者--即方法所属的对象--的实际类型，来决定该调用那种方法。
...
定义：将一个请求或命令封装为一个对象，从而可使用不同的请求对客户进行参数化；请求排队，记录请求日志，支持可撤销操作。
...
定义：多个观察者对象监听一个主题对象，当状态发生变化时，通知所有观察者对象，让它们自动更新自己。

优点：用于维护一组相关对象的一致性。

事件委托：

应用：通常与责任链模式结合，负责对事件的处理，其中某一个observer负责是否将事件进一步传递。
...
1. 记录快照（瞬时状态），维护类属性的历史
2. 存盘：对象实现序列化，直接存储在硬盘。游戏Tank的存档。
...
优点：把不变行为搬移到超类，去除子类中的重复代码，实现代码复用。

#### 状态模式

定义：当一个对象的内在状态改变时，允许改变其行为。也就是对象的行为取决于它的状态。

优点：解决一个对象状态转换的条件表达式很复杂的情况。把状态的判断逻辑转移到表示不同状态的一系列类当中，把复杂的判断逻辑简化。

应用：线程的有效状态机FSM，实现状态的迁移。

![image-20210702141004914](images/image-20210702141004914.png)

#### 策略模式

定义：封装算法，让它们可以相互替换。使算法的变化，不会影响到使用者。

优点：提供一系列封装的，可重用的算法或行为。
...
应用：对于发生频率搞定的问题，可以将该问题的各个实例表述为一个简单语言的句子，并通过构建的解释器，去解释这些语句来解决该问题。
